...
export const InviteManagerDrawer: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  const fetchInvites = useCallback(async () => {
    ...
    try {
      ...
      setError(getErrorMessage(error, 'Failed to load invitations.'));
    } finally {
      setIsLoading(false);
    }
  }, [activeTeam]);
  ...
  const handleSendInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(getErrorMessage(error, 'Failed to create invitation. Email might already be invited.'));
    } finally {
  ...
  const handleDelete = async (inviteId: number) => {
    ...
    try {
      ...
      setError(getErrorMessage(error, 'Failed to delete invitation.'));
    }
  };

  const copyToClipboard = async (invite: Invitation) => {
              ...
              invites.map((invite) => (
                <div key={invite.id} className="p-4 bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-sm space-y-3">
                  ...
                  <div className="flex items-center gap-2 p-2 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-100 dark:border-slate-800">
                    ...
                    <button
                      onClick={() => void copyToClipboard(invite)}
