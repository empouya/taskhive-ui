  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <h4
          className={`text-sm font-semibold truncate ${task.status === 'DONE' ? 'text-slate-400 line-through' : 'text-slate-900 dark:text-white'
            }`}
        >
          {task.title}
        </h4>
        ...
        <div className="flex items-center gap-3 mt-1">
          <span
            className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded ${priorityColors[task.priority]}`}
          >
            {task.priority}
          </span>
